...
switch (idx)
  ...
  case 3 : { return(g_strdup(driver_info->Version)); }
  default : return(NULL);
  }
}
...
if (msg->MessageType == MHS_MSG_PNP_EVENT)
  ...
  if (num_devs <= 0)
    {
    err = 1;
    UpdateMsgWidget(start_win->HardwareWdg, MSG_WDG_STATUS_WAIT, "<span size=\"xx-large\">Tiny-CAN Status</span>\n"
                                                 "<b>Tiny-CAN nicht verbunden</b>");
    }
  else
    {
    UpdateMsgWidget(start_win->HardwareWdg, MSG_WDG_STATUS_BUSY, "<span size=\"xx-large\">Tiny-CAN Status</span>\n"
                                               "<b>Initialisierung, Status unbekannt</b>");
    UpdateGtk();
    if (J1939CanOpen(start_win->CanAutobaud->CanCore) < 0)
      {
      err = 1;
      str = g_strdup_printf("<span size=\"xx-large\">Tiny-CAN Status</span>\n"
      ...
      {
      UpdateMsgWidget(start_win->HardwareWdg, MSG_WDG_STATUS_OK, "<span size=\"xx-large\">Tiny-CAN Status</span>\n"
  ...
  if (err)
    {
    UpdateMsgWidget(start_win->AutoBaudWdg, MSG_WDG_STATUS_STOP, "<span size=\"xx-large\">Automatische Baudratenerkennung</span>\n"
                                             "<b>CAN Schnittstelle nicht geöffnet</b>");
    }
  else
    {
    UpdateMsgWidget(start_win->AutoBaudWdg, MSG_WDG_STATUS_WAIT, "<span size=\"xx-large\">Automatische Baudratenerkennung</span>\n"
                                             "<b>Baudratenerkennung wird gestartet ...</b>");
    }
  UpdateGtk();
  if (err)
    {
    J1939Close(start_win->CanAutobaud->CanCore);
  ...
  {
  str = g_strdup_printf("<span size=\"xx-large\">Automatische Baudratenerkennung</span>\n"
                  "<b>%s</b>", msg_str);
...
if (can_core->AppCanStatus == APP_CAN_DRIVER_ERROR)
  {
  err = 1;
  str = g_strdup_printf("<span size=\"xx-large\">System</span>\n"
  ...
  {
  start_win->DriverWdg = CreateMsgWidget(MSG_WDG_STATUS_OK, "<span size=\"xx-large\">System</span>\n"
...
if (err)
  {
  start_win->HardwareWdg = CreateMsgWidget(MSG_WDG_STATUS_STOP, "<span size=\"xx-large\">Tiny-CAN Status</span>\n"
                                           "<b>Fataler Fehler: Treiber API nicht geladen</b>");
  }
else
  {
  start_win->HardwareWdg = CreateMsgWidget(MSG_WDG_STATUS_BUSY, "<span size=\"xx-large\">Tiny-CAN Status</span>\n"
                                           "<b>Initialisierung, Status unbekannt</b>");
  }
...
if (err)
  {
  start_win->AutoBaudWdg = CreateMsgWidget(MSG_WDG_STATUS_STOP, "<span size=\"xx-large\">Automatische Baudratenerkennung</span>\n"
                                           "<b>Fataler Fehler: Treiber API nicht geladen</b>");
  }
else
  {
  start_win->AutoBaudWdg = CreateMsgWidget(MSG_WDG_STATUS_BUSY, "<span size=\"xx-large\">Automatische Baudratenerkennung</span>\n"
